...
export const signup = async (req, res) => {
    ...
    try {
        if (!fullName || !email || !password) {
            return res.status(400).json({ mssage: "All fields are required" });
        }
        if (password.length < 6) {
            return res.status(400).json({ mssage: "Password must be at least 6 characters" });
        }

        const user = await User.findOne({ email });

        if (user) return res.status.json({ message: "This email already used" });
